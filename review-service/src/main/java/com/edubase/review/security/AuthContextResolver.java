...
public class AuthContextResolver {
    ...
    public AuthContext resolveOptional(Jwt jwt) {
        if (jwt == null) {
            return null;
        }
        ...
        return new AuthContext(userId, extractRole(jwt));
    }
    ...
    private UserRole extractRole(Jwt jwt) {
        ...
        boolean isUser = roles.stream().anyMatch(role -> UserRole.from(role) == UserRole.USER);
        if (isUser) {
