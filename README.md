...
MINIO_ROOT_USER=edubaseminio
MINIO_ROOT_PASSWORD=edubase-miniopass
USER_MEDIA_MINIO_ACCESS_KEY=edubaseminio
USER_MEDIA_MINIO_SECRET_KEY=edubase-miniopass
COURSE_MEDIA_MINIO_ACCESS_KEY=edubaseminio
COURSE_MEDIA_MINIO_SECRET_KEY=edubase-miniopass
```

Production veya paylaşımlı ortamlarda bu değerleri mutlaka değiştirin. Özellikle `JWT_SECRET`, MinIO credentialları, Gmail bilgileri ve dahili API anahtarı varsayılan haliyle kullanılmamalıdır.
...
`k8s/02-secret.yaml` dosyası lokal geliştirme için varsayılan değerler içerir. Gerçek ortamda bu dosyadaki değerleri doğrudan kullanmayın.
...
- Repository içindeki secret değerleri sadece lokal geliştirme için düşünülmelidir.
